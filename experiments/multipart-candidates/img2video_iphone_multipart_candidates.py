...
SCRIPT_BUILD = "interactive-five-level-final-frame-choice-v6"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=range(1, 7),
        default=parse_int(
            settings.get("maxParallelTasks", "3"), "maxParallelTasks", 1, 10
        ),
        metavar="1-6",
...
def multipart_prompt_candidates(
    prompt: str, limit: int = 10, output_separator: str = "; "
) -> list[str]:
    """Build up to ``limit`` ordered prompt combinations from semicolon parts.

    Part 1 is mandatory. Remaining parts are optional. Shorter combinations are
    tested before richer ones, while the complete prompt is always retained.
    A prompt without semicolons returns unchanged.
    """
    ...
    if len(parts) > 5:
        raise RuntimeError(
            f"A multipart prompt may contain at most 5 non-empty semicolon parts; got {len(parts)}: {prompt}"
        )
    candidates = []
    optional = list(range(1, len(parts)))
    for optional_count in range(0, len(optional) + 1):
        for chosen in itertools.combinations(optional, optional_count):
            indexes = (0,) + chosen
            candidates.append(output_separator.join(parts[index] for index in indexes))
    full = output_separator.join(parts)
    # Keep the complete prompt even when the combination space exceeds the cap.
...
def generate_first_completed_candidate(
    photo: Path,
    base_prompt: str,
    output: Path,
    parameters: dict,
    ...
    """Race multipart prompt variants and keep the first successful completion."""
    prompts = multipart_prompt_candidates(base_prompt, 10)
    if len(prompts) == 1:
        generate_video(
            photo, prompts[0], output, parameters, args.poll_interval,
            username, password,
    ...
    print(f"Multipart prompt: racing {len(prompts)} candidate(s).")
    image_data = encode_image(photo)
    existing = state.get("candidateTasks") or []
    jobs = []
    if existing:
        jobs = existing
        print(f"Resuming {len(jobs)} saved candidate task(s).")
    else:
        used_seeds = set()
        for index, candidate_prompt in enumerate(prompts):
            candidate_parameters = dict(parameters)
            seed = secrets.randbelow(2**31)
            while seed in used_seeds:
                seed = secrets.randbelow(2**31)
            used_seeds.add(seed)
            candidate_parameters["seed"] = seed
            jobs.append({
                "index": index,
        ...
        workers = min(args.max_parallel_tasks, len(jobs))
        print(f"Submitting candidates with up to {workers} parallel requests…")
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(
                    submit_encoded_task, image_data, job["prompt"],
                    job["parameters"], username, password
                ): job
                for job in jobs
            }
            for future in concurrent.futures.as_completed(futures):
                job = futures[future]
                task_id, started_at = future.result()
                job.update({"taskId": task_id, "startedAt": started_at, "status": "submitted"})
                print(
                    f"  candidate {job['index'] + 1}: task={task_id} "
                    f"seed={job['parameters']['seed']} | {job['prompt']}"
                )
        state["candidateTasks"] = jobs
        if state_path is not None:
            save_state(state_path, state)
    ...
    while winner is None:
        active = [job for job in jobs if job.get("status") not in TERMINAL_STATES]
        if not active:
            details = "; ".join(f"candidate {i}: {reason}" for i, reason in failures)
            raise TaskTerminalError(f"All multipart candidates failed. {details}")
        workers = min(args.max_parallel_tasks, len(active))
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(
                    api_request, "GET", f"/api/v3/tasks/{job['taskId']}",
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if status == "completed":
                    ...
                    break
                if status in TERMINAL_STATES:
                    failures.append((job["index"] + 1, status))
        state["candidateTasks"] = jobs
        state["lastActiveAt"] = time.time()
        state["lastStatus"] = "candidate_polling"
        if state_path is not None:
            save_state(state_path, state)
        if winner is None:
            time.sleep(args.poll_interval)

    winner_job, winner_task = winner
    print(
        f"Winner: candidate {winner_job['index'] + 1} "
        f"seed={winner_job['parameters']['seed']} | {winner_job['prompt']}"
    )
    download(completed_video_url(winner_task), output)
    print(f"Saved segment: {output}")

    # Stop queued/preparing losers to avoid unnecessary credit use where possible.
    for job in jobs:
        if job is winner_job or not job.get("taskId"):
            continue
        if job.get("status") in CANCELABLE_STATES or job.get("status") in {"submitted", "planned", "unknown"}:
            cancel_task(job["taskId"], username, password)
    state.pop("candidateTasks", None)
    if state_path is not None:
        save_state(state_path, state)



def extract_final_frame_pixels(video: Path, scratch: Path) -> tuple[bytes, Path]:
    """Extract the last frame and return normalized 256x256 grayscale pixels."""
    frame = scratch / f"{video.stem}_last.png"
    extract_last_frame(video, frame)
    try:
        completed = subprocess.run(
            [
                "ffmpeg", "-hide_banner", "-loglevel", "error", "-nostats", "-nostdin",
                "-i", str(frame), "-vf", "scale=256:256:flags=lanczos,format=gray",
                "-frames:v", "1", "-f", "rawvideo", "-",
            ],
            capture_output=True,
        )
    except FileNotFoundError as error:
        raise RuntimeError("FFmpeg is required for endpoint deduplication.") from error
    expected = 256 * 256
    if completed.returncode != 0 or len(completed.stdout) < expected:
        details = completed.stderr.decode("utf-8", errors="replace").strip()
        raise RuntimeError(f"Could not read final frame of {video.name}: {details or 'invalid frame data'}")
    return completed.stdout[:expected], frame


def frame_ssim(left: bytes, right: bytes) -> float:
    """Calculate global SSIM for two equally sized 8-bit grayscale frames."""
...
def keep_distinct_endings(results: list[dict], threshold: float = 0.9999) -> list[dict]:
    """Keep branches unless their actual final frames are nearly identical by SSIM."""
    kept = []
    for result in sorted(results, key=lambda item: item.get("completedOrder", 10**9)):
        duplicate = None
        similarity = 0.0
        for other in kept:
            score = frame_ssim(result["finalFramePixels"], other["finalFramePixels"])
            if score >= threshold:
                duplicate = other
                similarity = score
                break
        if duplicate is None:
            ...
            result["duplicateOf"] = duplicate["label"]
            result["endpointSsim"] = similarity
            print(
                f"  removed {result['label']} -> duplicate of {duplicate['label']} "
                f"(final-frame SSIM {similarity:.6f})"
            )
            for disposable in (result.get("output"), result.get("lastFrame")):
                if disposable:
                    try:
                        Path(disposable).unlink()
                    except FileNotFoundError:
                        pass
    return kept


def cumulative_prompt_levels(prompt: str) -> list[str]:
    """Return cumulative semicolon-part levels: P1, P1+P2, ... up to five."""
    parts = [part.strip() for part in prompt.split(";") if part.strip()]
    if not parts:
        raise RuntimeError("Prompt is empty.")
    if len(parts) > 5:
        raise RuntimeError(
            f"An interactive multipart prompt may contain at most 5 non-empty parts; got {len(parts)}: {prompt}"
        )
    return [", ".join(parts[:level]) for level in range(1, len(parts) + 1)]
...
def generate_candidate_level(
    ...
    password: str,
    *,
    cumulative_levels: bool = False,
    deduplicate: bool = True,
) -> list[dict]:
    """Generate every multipart variant for every supplied parent branch."""
    # Semicolons are only the source-file part separators. Level-2 API prompts
    # use natural comma-separated prose, as requested.
    separator = ", " if level == 2 else "; "
    prompts = (
        cumulative_prompt_levels(base_prompt)
        if cumulative_levels
        else multipart_prompt_candidates(base_prompt, 10, output_separator=separator)
    )
    output_directory.mkdir(parents=True, exist_ok=True)
    jobs = []
    used_seeds = set()
    image_cache = {}
    for parent_index, source in enumerate(sources, 1):
        photo = Path(source["photo"])
        key = str(photo)
        if key not in image_cache:
            image_cache[key] = encode_image(photo)
        for candidate_index, candidate_prompt in enumerate(prompts, 1):
            seed = secrets.randbelow(2**31)
            while seed in used_seeds:
                seed = secrets.randbelow(2**31)
            used_seeds.add(seed)
            candidate_parameters = dict(parameters)
            candidate_parameters["seed"] = seed
            label = f"L{level}_P{parent_index:02d}_C{candidate_index:02d}"
            jobs.append({
                "label": label,
                "parent": source,
                "photo": photo,
                "imageData": image_cache[key],
                "prompt": candidate_prompt,
                "parameters": candidate_parameters,
                "seed": seed,
                "output": output_directory / f"{label}.mp4",
                "status": "planned",
            })

    print(
        f"Level {level}: submitting {len(jobs)} candidate(s) "
        f"({len(sources)} parent branch(es) × {len(prompts)} prompt variant(s))."
    )
    workers = min(args.max_parallel_tasks, len(jobs))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
        futures = {
            executor.submit(
                submit_encoded_task, job["imageData"], job["prompt"],
                job["parameters"], username, password,
            ): job
            for job in jobs
        }
        for future in concurrent.futures.as_completed(futures):
            ...
            task_id, started_at = future.result()
            job.update({"taskId": task_id, "startedAt": started_at, "status": "submitted"})
            print(f"  {job['label']}: task={task_id} seed={job['seed']} | {job['prompt']}")

    pending = list(jobs)
    completed_results = []
    completion_counter = 0
    while pending:
        active = [job for job in pending if job["status"] not in TERMINAL_STATES]
        if not active:
            break
        workers = min(args.max_parallel_tasks, len(active))
        with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, workers)) as executor:
            futures = {
                executor.submit(
                    api_request, "GET", f"/api/v3/tasks/{job['taskId']}", username, password
                ): job
                for job in active
            }
            for future in concurrent.futures.as_completed(futures):
                ...
                if status == "completed":
                    ...
                    download(completed_video_url(task), job["output"], show_progress=False)
                    job["completedOrder"] = completion_counter
                    completed_results.append(job)
                    pending.remove(job)
                    print(f"  completed {job['label']} -> {job['output'].name}")
                elif status in TERMINAL_STATES:
                    pending.remove(job)
                    print(f"  rejected {job['label']}: {status}")
        if pending:
            time.sleep(args.poll_interval)

    if not completed_results:
        raise TaskTerminalError(f"Every level-{level} candidate failed.")

    scratch = output_directory / "_frames"
    scratch.mkdir(exist_ok=True)
    for result in completed_results:
        result["finalFramePixels"], result["lastFrame"] = extract_final_frame_pixels(result["output"], scratch)
    if not deduplicate:
        completed_results.sort(key=lambda item: item["label"])
        print(f"Level {level}: saved {len(completed_results)} candidate video(s) and final frame(s).")
        return completed_results

    distinct = keep_distinct_endings(completed_results, threshold=0.9999)
    print(
        f"Level {level}: kept {len(distinct)}/{len(completed_results)} "
        "distinct ending frame(s) (final-frame SSIM < 0.9999)."
    )
    return distinct


def run_two_level_window(
    current_photo: Path,
    first_prompt: str,
    second_prompt: str | None,
    first_segment: Path,
    second_segment: Path | None,
    parameters: dict,
    args,
    username: str,
    password: str,
    ...
    """Explore at most two prompt levels, retain all distinct endpoints, and select one path."""
    window_dir = branch_root / f"window_{window_number:02d}"
    level1 = generate_candidate_level(
        [{"label": "ROOT", "photo": current_photo}], first_prompt,
        window_dir / "level_1", 1, parameters, args, username, password,
    )
    if second_prompt is None:
        selected = level1[0]
        shutil.copy2(selected["output"], first_segment)
        print(f"Selected distinct branch: {selected['label']}")
        return 1

    level2_sources = [
        {"label": item["label"], "photo": item["lastFrame"], "level1": item}
        for item in level1
    ]
    level2 = generate_candidate_level(
        level2_sources, second_prompt, window_dir / "level_2", 2,
        parameters, args, username, password,
    )
    selected = level2[0]
    selected_level1 = selected["parent"]["level1"]
    shutil.copy2(selected_level1["output"], first_segment)
    shutil.copy2(selected["output"], second_segment)
    manifest = {
        "selected": {
            "level1": selected_level1["label"],
            "level2": selected["label"],
        },
        "distinctLevel1": [item["label"] for item in level1],
        "distinctLevel2": [item["label"] for item in level2],
    }
    (window_dir / "branches.json").write_text(
        json.dumps(manifest, indent=2), encoding="utf-8"
    )
    print(
        f"Selected two-level path: {selected_level1['label']} -> {selected['label']}\n"
        f"Retained {len(level1)} distinct level-1 and {len(level2)} distinct level-2 branch(es) in {window_dir}"
    )
    return 2


def run_all_distinct_possibilities(
    prompts: list[str],
    original_photo: Path,
    parameters: dict,
    args,
    ...
    """Continue every visually distinct endpoint through successive two-prompt windows."""
    branch_root = final_output.with_name(final_output.stem + "_branches")
    ...
    while prompt_index < len(prompts):
        ...
        level1_sources = [
            {"label": branch["label"], "photo": branch["photo"], "branch": branch}
            for branch in frontier
        ]
        level1 = generate_candidate_level(
            level1_sources, first_prompt, window_dir / "level_1", 1,
            parameters, args, username, password,
        )
        level1_frontier = []
        for item in level1:
            parent_branch = item["parent"]["branch"]
            level1_frontier.append({
                "label": item["label"],
                "photo": item["lastFrame"],
                "segments": parent_branch["segments"] + [Path(item["output"])],
                "lineage": parent_branch["lineage"] + [item["label"]],
            })

        if prompt_index + 1 < len(prompts):
            second_prompt = prompts[prompt_index + 1]
            level2_sources = [
                {"label": branch["label"], "photo": branch["photo"], "branch": branch}
                for branch in level1_frontier
            ]
            level2 = generate_candidate_level(
                level2_sources, second_prompt, window_dir / "level_2", 2,
                parameters, args, username, password,
            ...
            frontier = level1_frontier
            consumed = 1

        manifest = {
            "window": window_number,
            "promptIndexes": list(range(prompt_index + 1, prompt_index + consumed + 1)),
            "survivingBranches": [
                {"label": branch["label"], "lineage": branch["lineage"]}
                for branch in frontier
            ],
        }
        (window_dir / "branches.json").write_text(
        ...
        print(
            f"Window {window_number}: {len(frontier)} visually distinct branch(es) "
            "will all continue."
        )
        prompt_index += consumed
        window_number += 1

    if not frontier:
        raise TaskTerminalError("No complete distinct branch survived.")

    outputs = []
    output_manifest = []
    for branch_index, branch in enumerate(frontier, 1):
        if branch_index == 1:
            output = final_output
        else:
            output = final_output.with_name(
                f"{final_output.stem}_possibility_{branch_index:03d}{final_output.suffix}"
            )
        combine_targets = list(branch["segments"])
        if source_video and args.include_source_video:
            lead_in = prepare_lead_in_clip(
                Path(source_video), combine_targets[0],
                effective_output_fps(parameters), temporary_directory,
            )
            combine_targets = [lead_in] + combine_targets
        if len(combine_targets) > 1:
            combine_segments(
                combine_targets, output, temporary_directory,
                effective_output_fps(parameters),
            )
        else:
            shutil.copy2(combine_targets[0], output)
        outputs.append(output)
        output_manifest.append({
            "output": str(output),
            "lineage": branch["lineage"],
            "segments": [str(value) for value in branch["segments"]],
        })
        print(f"Saved possibility {branch_index}/{len(frontier)}: {output}")

    (branch_root / "possibilities.json").write_text(
        json.dumps(output_manifest, indent=2), encoding="utf-8"
    )
    return outputs

def run_interactive_prompt_choices(
    prompts: list[str],
    original_photo: Path,
    parameters: dict,
    args,
    ...
    """Generate cumulative prompt levels, pause for visual choice, and continue."""
    choice_root = final_output.with_name(final_output.stem + "_choices")
    choice_root.mkdir(parents=True, exist_ok=True)
    current_photo = original_photo
    selected_segments: list[Path] = []
    choices_manifest = []

    for prompt_index, base_prompt in enumerate(prompts, 1):
        prompt_dir = choice_root / f"prompt_{prompt_index:02d}"
        candidates = generate_candidate_level(
            [{"label": "ROOT", "photo": current_photo}],
            base_prompt, prompt_dir, prompt_index, parameters, args, username, password,
            cumulative_levels=True, deduplicate=False,
        )
        candidates.sort(key=lambda item: item["label"])

        print(f"\nPrompt {prompt_index}/{len(prompts)} candidates are ready.")
        for candidate_number, candidate in enumerate(candidates, 1):
            # Rename the extracted image to a short predictable name beside its video.
            final_frame = prompt_dir / f"C{candidate_number:02d}_final.png"
            if Path(candidate["lastFrame"]) != final_frame:
                shutil.copy2(candidate["lastFrame"], final_frame)
            candidate["choiceFrame"] = final_frame
            print(
                f"  {candidate_number}. C{candidate_number:02d} | {candidate['prompt']}\n"
                f"     video: {candidate['output']}\n"
                f"     final: {final_frame}"
            )
        ...
        while True:
            try:
                raw_choice = input(
                    f"Open the final-frame PNGs in Files, then choose candidate [1-{len(candidates)}]: "
                ).strip()
            except EOFError as error:
                raise RuntimeError("Interactive candidate selection requires terminal input.") from error
            if raw_choice.isdigit() and 1 <= int(raw_choice) <= len(candidates):
                selected_index = int(raw_choice) - 1
                break
            print(f"Enter a number from 1 to {len(candidates)}.")

        selected = candidates[selected_index]
        selected_segments.append(Path(selected["output"]))
        current_photo = Path(selected["choiceFrame"])
        choices_manifest.append({
            "promptIndex": prompt_index,
            "basePrompt": base_prompt,
            "selectedCandidate": selected_index + 1,
            "selectedPrompt": selected["prompt"],
            "selectedVideo": str(selected["output"]),
            "selectedFinalFrame": str(current_photo),
            "candidates": [
                {
                    "candidate": number,
                    "prompt": candidate["prompt"],
                    "video": str(candidate["output"]),
                    "finalFrame": str(candidate["choiceFrame"]),
                    "seed": candidate["seed"],
                }
                for number, candidate in enumerate(candidates, 1)
            ],
        })
        (choice_root / "choices.json").write_text(
            json.dumps(choices_manifest, indent=2), encoding="utf-8"
        )
        print(f"Selected C{selected_index + 1:02d}; its final frame will start the next prompt.")

    combine_targets = list(selected_segments)
    if source_video and args.include_source_video:
        lead_in = prepare_lead_in_clip(
            Path(source_video), combine_targets[0],
            effective_output_fps(parameters), temporary_directory,
    ...
    if len(combine_targets) > 1:
        combine_segments(
            combine_targets, final_output, temporary_directory,
            effective_output_fps(parameters),
        )
    else:
        shutil.copy2(combine_targets[0], final_output)
    print(f"Saved interactively selected chain: {final_output}")
    return [final_output]
...
def run_plan(
    ...
    with tempfile.TemporaryDirectory(
        ...
        # Multipart prompts are handled interactively, one prompt at a time.
        # Each semicolon part creates the next cumulative level (up to five).
        # All videos and final PNGs are retained; the user's chosen final frame
        # becomes the source for the next prompt.
        if any(";" in prompt for prompt in prompts):
            run_interactive_prompt_choices(
                prompts, original_photo, parameters, args, username, password,
                final_output, state.get("sourceVideo"), temporary_directory,
            )
            if state_path is not None and state_path.exists():
                state_path.unlink()
                print(f"Recovery state cleared: {state_path.name}")
        ...
        while int(state["currentIndex"]) < len(prompts):
            ...
            # Multipart prompts are explored in bounded two-level windows. All
            # visually distinct endpoints are retained; only one path is copied
            # into the main assembled chain.
            if ";" in step_prompt and not task_id:
                next_prompt = prompts[index + 1] if index + 1 < len(prompts) else None
                next_segment = segments[index + 1] if index + 1 < len(segments) else None
                branch_root = final_output.with_name(final_output.stem + "_branches")
                consumed = run_two_level_window(
                    current_photo, step_prompt, next_prompt, segment, next_segment,
                    step_parameters, args, username, password, branch_root, index // 2 + 1,
                )
                state["currentIndex"] = index + consumed
                state.pop("candidateTasks", None)
                state["taskId"] = None
                state["taskStartedAt"] = None
                state["lastActiveAt"] = time.time()
                state["lastStatus"] = "two_level_window_completed"
                if state_path is not None:
                    save_state(state_path, state)
                continue
